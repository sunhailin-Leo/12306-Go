...
var output = NewColorIO(os.Stdout)
...
func (t *Table) printHeading() {
	...
	fmt.Fprint(output, ConditionString(t.borders.Left, t.pColumn, SPACE))
...
func (t *Table) printFooter() {
	...
	// Check if border is set
	// Replace with space if not set
	fmt.Fprint(output, ConditionString(t.borders.Bottom, t.pColumn, SPACE))
...
func (t *Table) printRow(columns [][]string, colKey int) {
	...
	for x := 0; x < max; x++ {
		for y := 0; y < total; y++ {
			...
			switch t.align {
			case ALIGN_CENTER: //
				fmt.Fprintf(output, "%s", Pad(str, SPACE, t.cs[y]))
			case ALIGN_RIGHT:
				fmt.Fprintf(output, "%s", PadLeft(str, SPACE, t.cs[y]))
			case ALIGN_LEFT:
				fmt.Fprintf(output, "%s", PadRight(str, SPACE, t.cs[y]))
			default:
